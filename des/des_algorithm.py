...
for x in str2bin(text):
    ...
    if len(unit)==64:
        ...
        unit=[]
#print 64*len(bindepart)+len(unit)
#unit中保留多余部分

for i,part in enumerate(bindepart):
    part=replace(part,[x-1 for x in ip])
    print '\n'
    lpart=part[:32]
    rpart=part[32:]
    for a in range(16):
        lpart,rpart=text_op(lpart,rpart,a)
    #print max(ipinv),len(temp),i
    bindepart[i]=replace(rpart+lpart,[x-1 for x in ipinv])
